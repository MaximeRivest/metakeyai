...
function setupIpcListeners(): void {
  ...
  ipcMain.on('test-voice', async (event, { voice, text }) => {
    ...
    // Generalized recording system that can work with any window
  ...
  function createRecordingSession(params: {
    sessionId: string;
    window: BrowserWindow | null;
    filePrefix: string;
    startEvent: string;
    ...
    return {
      id: params.sessionId,
      isRecording: false,
      outputFile: path.join(tmpdir(), `${params.filePrefix}_${Date.now()}.webm`),
      window: params.window,
      cleanupHandlers: null,
      startEvent: params.startEvent,
      finishedEvent: params.finishedEvent,
  ...
  async function startRecordingSession(sessionId: string): Promise<{ success: boolean, filePath?: string, error?: string }> {
    ...
    console.log(`📁 Recording file path: ${session.outputFile}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      if (session.window) {
        console.log(`🎤 Sending start command to window for session: ${sessionId}`);
        session.window.webContents.send(session.startCommand);
      } else {
        session.cleanupHandlers();
        session.cleanupHandlers = null;
        reject(new Error(`Window not available for session: ${sessionId}`));
      }
    });
  }

  async function stopRecordingSession(sessionId: string): Promise<{ success: boolean, error?: string }> {
    const session = activeSessions.get(sessionId);
    if (!session) {
      return { success: false, error: 'Session not found' };
    }
  ...
  // Helper functions for common recording scenarios
  function getWindowByType(windowType: 'first-run' | 'settings' | 'pastille' | 'main'): BrowserWindow | null {
    switch (windowType) {
      case 'first-run':
  ...
  function createStandardRecordingParams(sessionId: string, windowType: string, options?: {
    filePrefix?: string;
    eventPrefix?: string;
    customWindow?: BrowserWindow;
  }) {
    const eventPrefix = options?.eventPrefix || sessionId;
    return {
      sessionId,
      window: options?.customWindow || getWindowByType(windowType as any),
      filePrefix: options?.filePrefix || `${sessionId}_recording`,
  ...
  // General recording session handlers
  ipcMain.handle('start-recording-session', async (event, params: {
    ...
    const { sessionId, windowType, customWindow } = params;
    
    // Create session using the helper function
    const session = createRecordingSession(
      createStandardRecordingParams(sessionId, windowType, {
    ...
    try {
      return await startRecordingSession(sessionId);
    } catch (error) {
      console.error(`❌ Failed to start recording session ${sessionId}:`, error);
  ...
  ipcMain.handle('stop-recording-session', async (event, sessionId: string) => {
    try {
      return await stopRecordingSession(sessionId);
    } catch (error) {
      console.error(`❌ Failed to stop recording session ${sessionId}:`, error);
      return { success: false, error: (error as Error).message };
    }
  });

  // First-run setup recording handlers (backwards compatibility)
  ipcMain.handle('start-first-run-recording', async () => {
    ...
    const session = createRecordingSession({
      sessionId,
      window: firstRunManager?.getSetupWindow() || null,
      filePrefix: 'first_run_recording',
      startEvent: 'first-run-audio-started',
      finishedEvent: 'first-run-audio-finished',
      errorEvent: 'first-run-audio-error',
      startCommand: 'start-first-run-recording',
      stopCommand: 'stop-first-run-recording'
    });
    ...
    try {
      ...
      console.error('❌ Failed to start first-run recording:', error);
      return { success: false, error: (error as Error).message };
    }
  });
  ...
  ipcMain.handle('stop-first-run-recording', async () => {
    try {
      return await stopRecordingSession('first-run');
    } catch (error) {
      console.error('❌ Failed to stop first-run recording:', error);
      return { success: false, error: (error as Error).message };
    }
  });

  ipcMain.handle('play-first-run-recording', async (event, filePath: string) => {
    console.log('🔊 Playing first-run recording:', filePath);
    try {
      if (!audioPlayer) {
        audioPlayer = new AudioPlayer();
      }
      ...
      console.error('❌ Failed to play first-run recording:', error);
      return { success: false, error: (error as Error).message };
    }
  });

  ipcMain.handle('stop-first-run-playback', async () => {
    console.log('🛑 Stopping first-run playback...');
    try {
      if (audioPlayer) {
        audioPlayer.stop();
        return { success: true };
      ...
      console.error('❌ Failed to stop first-run playback:', error);
      return { success: false, error: (error as Error).message };
    }
  });
  ...
  // UserDataManager access - separate handlers to avoid cloning issues
