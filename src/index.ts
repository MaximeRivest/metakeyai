...
// Pastille window dimensions
const PASTILLE_WINDOW_WIDTH = 624;  // 520 + 20% buffer (52px on each side)
const PASTILLE_WINDOW_HEIGHT = 150; // 50px * 3 for breathing room
const PASTILLE_COLLAPSED_HEIGHT = 50; // Height when collapsed to normal mode
...
const handleShowClipboard = () => {
  ...
  if (pastilleWindow && pastilleWindow.isVisible()) {
    ...
    if (pastilleWindow.isFocused() || pastilleWindow.isAlwaysOnTop()) {
      console.log('👁️ Pastille already visible and in front - hiding it');
      pastilleWindow.hide();
    } else {
      console.log('👁️ Pastille visible but behind other windows - bringing to front');
...
const showPastille = () => {
  ...
  // Only set position if window is being shown for the first time
  // Position is now managed by user preferences and dragging only
  console.log('👁️ Showing pastille at saved position');
  ...
  console.log('👁️ Pastille window shown');
};
...
const handleQuickEdit = async (): Promise<void> => {
  ...
  if (!setupStatus.isConfigured) {
    console.log('⚠️ Python not configured for Quick Edit');
    
    pastilleWindow?.webContents.send('show-message', 'Python setup required for Quick Edit. Check Settings → Python Setup.');
    pastilleWindow?.show();
  ...
  try {
    ...
    if (session.success) {
      ...
      const finishHandler = (filePath: string) => {
        console.log('✅ Voice recording finished, file:', filePath);
      ...
      console.error('❌ Failed to start voice recording:', session.error);
      pastilleWindow?.webContents.send('show-message', `Recording Error – ${session.error}`);
      pastilleWindow?.show();
    }
  } catch (error) {
    console.error('❌ Voice recording error:', error);
    pastilleWindow?.webContents.send('show-message', `Recording Setup Error – ${(error as Error).message}`);
    pastilleWindow?.show();
  }
};
...
const createMainWindow = () => {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    if (mainWindow && !mainWindow.isVisible()) {
      mainWindow.show();
      mainWindow.focus();
      console.log('🐧 Main window forced to show on Linux');
    }
  });
...
async function cleanupApplication(): Promise<void> {
  ...
  try {
    ...
    if (firstRunManager) {
      ...
      console.log('✅ First run manager cleaned up');
    }
  } catch (error) {
    console.error('❌ Error cleaning up first run manager:', error);
  }
...
const showPillNotification = (message: string) => {
  ...
  // Only show message, don't move the pastille
...
const showSpellCompletionFeedback = (result: any) => {
  ...
  setTimeout(() => {
    if (clipboardHistory) {
      ...
      if (newestEntry) {
        console.log('🎯 📋 Auto-navigating to spell result:', newestEntry.text.substring(0, 50) + '...');
    ...
    const debugMenu = Menu.buildFromTemplate([
      {
        ...
        submenu: [
          ...
          },
          {
            ...
            click: async () => {
              try {
                logger.log('🎤 Testing audio recording via AudioManager...');
                if (audioManager) {
                  const result = await audioManager.testMicrophoneDevice('auto');
                  logger.log(`✅ Audio test completed: ${result.success ? 'SUCCESS' : 'FAILED'}`);
                  dialog.showMessageBox({
          ...
          {
            label: 'Reset Pastille Position',
            click: () => {
              console.log('📍 Resetting pastille position from debug menu');
              if (pastilleWindow) {
                const primaryDisplay = screen.getPrimaryDisplay();
                const { width, height } = primaryDisplay.workAreaSize;
                const x = Math.round((width - PASTILLE_WINDOW_WIDTH) / 2);
                const y = Math.round((height - PASTILLE_WINDOW_HEIGHT) / 2);
                pastilleWindow.setPosition(x, y);
                
                // Save the new position
                userDataManager?.savePastillePosition({ x, y });
                
                // Show the pastille so user can see it's centered
                showPastille();
                
                console.log(`📍 Pastille centered at: ${x}, ${y}`);
                dialog.showMessageBox({
                  type: 'info',
                  title: 'Pastille Position Reset',
                  message: `Pastille centered at: ${x}, ${y}`,
                  buttons: ['OK']
                });
              }
            }
...
async function initializeApp() {
  ...
  console.log('✅ Audio manager initialized with global IPC handlers');

  // Setup essential IPC handlers early (needed by first-run setup)
  console.log('🔧 Setting up essential IPC handlers...');
  setupEssentialIpcHandlers();
  console.log('✅ Essential IPC handlers set up');

  // Initialize first run manager
  console.log('🎬 Initializing first run manager...');
  firstRunManager = new FirstRunManager(userDataManager, async () => {
    // Initialize normal app after first-run setup is complete
    console.log('🎉 First-run complete, initializing normal app...');
    await initializeNormalApp(true);
  });
  console.log('✅ First run manager initialized');
  ...
  console.log('🧙‍♂️ Checking Python setup configuration...');
  try {
    const pythonSetup = PythonSetupManager.getInstance();
    ...
    if (!hasConfiguration) {
      console.log('⚠️ No Python configuration found - spell caster will initialize on demand');
      
      // Create spell caster but don't initialize it yet
      spellCaster = pythonSpellCaster;
      console.log('✅ Spell Caster ready for on-demand initialization');
    } else {
      console.log('✅ Python configuration found - initializing spell caster...');
      
      // Check detailed setup status and initialize
      const setupStatus = await pythonSetup.checkSetupStatus();
      
      if (!setupStatus.isConfigured) {
        console.log('⚠️ Python configuration exists but not properly configured');
        spellCaster = pythonSpellCaster;
        await spellCaster.initializeFallback();
        console.log('⚠️ Spell Caster initialized with limited functionality');
      } else {
        console.log('✅ Python environment verified - initializing full functionality');
        spellCaster = pythonSpellCaster;
    ...
    console.error('❌ Error during Python setup check:', error);
    // Create spell caster for on-demand initialization
    spellCaster = pythonSpellCaster;
    console.log('⚠️ Spell Caster ready for on-demand initialization (after error)');
  }
  
  // Create the pastille window
  console.log('🪟 Creating pastille window...');
  pastilleWindow = new BrowserWindow({
    width: PASTILLE_WINDOW_WIDTH,  // 520 + 20% buffer (52px on each side)
    height: PASTILLE_WINDOW_HEIGHT, // 50px * 3 for more breathing room
    frame: false,
    transparent: true,
    alwaysOnTop: false,
    show: false,
  ...
  const savedPosition = userDataManager?.loadPastillePosition();
  if (savedPosition) {
    // Validate position is still on screen (handle monitor changes)
    const displays = screen.getAllDisplays();
    let validPosition = false;
    let targetDisplay = null;
    
    // First, try to find the exact display it was saved on
    if (savedPosition.displayId) {
      targetDisplay = displays.find(d => d.id === savedPosition.displayId);
    }
    
    // If that display exists, check if position is valid
    if (targetDisplay) {
      const { x: dx, y: dy, width: dw, height: dh } = targetDisplay.workArea;
      if (savedPosition.x >= dx && savedPosition.x <= dx + dw - PASTILLE_WINDOW_WIDTH && 
          savedPosition.y >= dy && savedPosition.y <= dy + dh - PASTILLE_WINDOW_HEIGHT) {
        validPosition = true;
        console.log(`📍 Saved position is valid on original display ${targetDisplay.id}`);
      }
    }
    
    // If not valid on original display, check all displays
    if (!validPosition) {
      for (const display of displays) {
        ...
        if (savedPosition.x >= dx && savedPosition.x <= dx + dw - PASTILLE_WINDOW_WIDTH && 
          ...
          console.log(`📍 Saved position is valid on display ${display.id}`);
          break;
        }
      }
    }
    
    if (validPosition && targetDisplay) {
      pastilleWindow.setPosition(savedPosition.x, savedPosition.y);
      console.log(`📍 Pastille positioned at saved location: ${savedPosition.x}, ${savedPosition.y} on display ${targetDisplay.id}`);
    } else {
      // Position is off-screen, use primary display center
      const primaryDisplay = screen.getPrimaryDisplay();
      const { x: displayX, y: displayY, width, height } = primaryDisplay.workArea;
      const x = displayX + Math.round((width - PASTILLE_WINDOW_WIDTH) / 2);
      const y = displayY + Math.round((height - PASTILLE_WINDOW_HEIGHT) / 2);
      pastilleWindow.setPosition(x, y);
      
      // Save this new valid position
      userDataManager?.savePastillePosition({ x, y, displayId: primaryDisplay.id });
      console.log(`📍 Saved position off-screen, using primary display center: ${x}, ${y} on display ${primaryDisplay.id}`);
    }
  } else {
    // No saved position, use primary display center as default
    const primaryDisplay = screen.getPrimaryDisplay();
    const { x: displayX, y: displayY, width, height } = primaryDisplay.workArea;
    const x = displayX + Math.round((width - PASTILLE_WINDOW_WIDTH) / 2);
    const y = displayY + Math.round((height - PASTILLE_WINDOW_HEIGHT) / 2);
    ...
    // Save this default position with display info
    userDataManager?.savePastillePosition({ x, y, displayId: primaryDisplay.id });
    console.log(`📍 No saved position, using primary display center: ${x}, ${y} on display ${primaryDisplay.id}`);
  }
  
  // Save position when window is moved (dragged)
  pastilleWindow.on('moved', () => {
    const [x, y] = pastilleWindow.getPosition();
    ...
    userDataManager?.savePastillePosition(position);
    console.log(`📍 🎯 Position saved: ${x}, ${y} on display ${display?.id || 'unknown'}`);
  });
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    { type: 'separator' },
    { label: 'Reset Pastille Position', type: 'normal', click: () => {
        console.log('📍 Resetting pastille position to center');
        if (pastilleWindow) {
          // Use current display instead of primary display
          const [currentX, currentY] = pastilleWindow.getPosition();
          ...
          // Save the new position with display info
          userDataManager?.savePastillePosition({ x, y, displayId: currentDisplay.id });
          
          // Show the pastille so user can see it's centered
          showPastille();
          
          console.log(`📍 Pastille centered at: ${x}, ${y} on display ${currentDisplay.id}`);
        }
      }
    },
  ...
  if (firstRunManager) {
    console.log('🎬 Checking if first-run setup should be shown...');
    const wasFirstRun = await firstRunManager.isFirstRun();
    console.log('🔍 Was first run?', wasFirstRun);
    
    if (wasFirstRun) {
      console.log('👋 First run detected, delaying normal app initialization...');
      console.log('🎬 First-run setup will initialize features as needed');
      await firstRunManager.handleAppReady();
      // Normal app initialization will be called from onSetupComplete callback
    } else {
      console.log('✅ Not first run, starting normal initialization...');
      await initializeNormalApp();
    }
  } else {
    // No first run manager, start normal initialization
    console.log('⚠️ No first run manager, starting normal initialization...');
    await initializeNormalApp();
  }
}

async function initializeNormalApp(isFirstRunComplete = false): Promise<void> {
  console.log('🚀 Starting normal app initialization...');
...
function setupEssentialIpcHandlers(): void {
  ...
  try {
    ...
    ipcMain.handle('load-env', () => {
      console.log('🔧 IPC: load-env called');
      return readEnvConfig();
    });

    ipcMain.handle('save-env', async (event, { env, llm, llms }) => {
      console.log('🔧 IPC: save-env called');
      try {
        ...
        try {
          ...
          console.warn('⚠️ Could not notify daemon of env changes:', (error as Error).message);
    ...
    ipcMain.on('test-voice', async (event, { voice, text }) => {
      console.log('🔊 Testing voice from setup/settings:', voice);
      try {
    ...
    ipcMain.handle('update-user-settings', (event, partialSettings) => {
      console.log('🔧 IPC: update-user-settings called');
      userDataManager?.updateSettings(partialSettings);
      return { success: true };
    });

    ipcMain.handle('load-audio-settings', () => {
      console.log('🔧 IPC: load-audio-settings called');
      return userDataManager?.loadAudioSettings();
    });

    ipcMain.handle('save-audio-settings', (event, settings) => {
      console.log('🔧 IPC: save-audio-settings called');
      userDataManager?.saveAudioSettings(settings);
      return { success: true };
    });

    // Pastille position reset handler
    ipcMain.handle('reset-pastille-position', () => {
      console.log('📍 IPC: reset-pastille-position called');
      if (pastilleWindow) {
        const primaryDisplay = screen.getPrimaryDisplay();
        const { width, height } = primaryDisplay.workAreaSize;
        const x = Math.round((width - PASTILLE_WINDOW_WIDTH) / 2);
      ...
      return { success: false, error: 'Pastille window not available' };
    });
    ...
    console.error('❌ Failed to setup essential IPC handlers:', error);
    throw error;
  }
}
...
function setupIpcListeners(): void {
  ...
  ipcMain.on('expand-pastille', () => {
    if (pastilleWindow) {
      console.log('📖 Expanding pastille window for edit mode');
      
      // Get current window position and find which display it's on
      const [currentX, currentY] = pastilleWindow.getPosition();
      const currentDisplay = screen.getDisplayNearestPoint({ x: currentX, y: currentY });
      ...
      console.log(`📖 💾 Saving collapsed position: ${currentX}, ${currentY}`);
      userDataManager?.savePastillePosition({ x: currentX, y: currentY, displayId: currentDisplay.id });
      
      // Calculate expanded size (80% of current monitor)
      const targetWidth = Math.round(screenWidth * 0.8);
      const targetHeight = Math.round(screenHeight * 0.8);
      
      // Center on the current monitor (not primary)
      const centerX = displayX + Math.round((screenWidth - targetWidth) / 2);
      const centerY = displayY + Math.round((screenHeight - targetHeight) / 2);
      
      // First resize the window
      pastilleWindow.setSize(targetWidth, targetHeight, true);
      
      // Position on current monitor
      pastilleWindow.setPosition(centerX, centerY);
      
      // Bring to front and make it always on top while editing
      pastilleWindow.setAlwaysOnTop(true);
      pastilleWindow.show();
      pastilleWindow.focus();
      
      console.log(`📖 Pastille expanded to ${targetWidth}x${targetHeight} at ${centerX}, ${centerY}`);
    }
  });

  // Add fallback IPC listener for force-window-resize
  ipcMain.on('force-window-resize', (event, { width, height }) => {
    if (pastilleWindow) {
      console.log('📖 🔄 FALLBACK: Force resizing pastille window to:', width, 'x', height);
      ...
      // Save current collapsed position before expanding
      console.log(`📖 🔄 💾 FALLBACK: Saving collapsed position: ${currentX}, ${currentY}`);
      userDataManager?.savePastillePosition({ x: currentX, y: currentY, displayId: currentDisplay.id });
      
      // Force resize
      pastilleWindow.setSize(width, height, true);
      
      // Center on the current monitor (not primary)
      const centerX = displayX + Math.round((screenWidth - width) / 2);
      const centerY = displayY + Math.round((screenHeight - height) / 2);
      pastilleWindow.setPosition(centerX, centerY);
      ...
      console.log(`📖 🔄 FALLBACK: Pastille force-resized to ${width}x${height} at ${centerX}, ${centerY} on display ${currentDisplay.id}`);
    }
  });
  ...
  ipcMain.on('collapse-pastille', () => {
    if (pastilleWindow) {
      console.log('📕 Collapsing pastille window from edit mode');
      
      // First resize back to original size
      pastilleWindow.setSize(PASTILLE_WINDOW_WIDTH, PASTILLE_WINDOW_HEIGHT, true);
      
      // Load saved position (should have been saved during expand)
      const savedPosition = userDataManager?.loadPastillePosition();
      if (savedPosition) {
        console.log(`📕 📁 Found saved position: ${savedPosition.x}, ${savedPosition.y} on display ${savedPosition.displayId}`);
        
        // Validate position is still on screen (handle monitor disconnection)
        const displays = screen.getAllDisplays();
        let validPosition = false;
        let targetDisplay = null;
        ...
        if (validPosition && targetDisplay) {
          pastilleWindow.setPosition(savedPosition.x, savedPosition.y);
          console.log(`📕 📍 Restored pastille to saved position: ${savedPosition.x}, ${savedPosition.y} on display ${targetDisplay.id}`);
        } else {
          // Position is off-screen on all displays, center on current display
          const [currentX, currentY] = pastilleWindow.getPosition();
          const currentDisplay = screen.getDisplayNearestPoint({ x: currentX, y: currentY });
          const { x: displayX, y: displayY, width, height } = currentDisplay.workArea;
          const x = displayX + Math.round((width - PASTILLE_WINDOW_WIDTH) / 2);
          ...
          // Save this new valid position
          userDataManager?.savePastillePosition({ x, y, displayId: currentDisplay.id });
          console.log(`📕 ⚠️ Saved position off-screen, centered on current display ${currentDisplay.id} at: ${x}, ${y}`);
        }
      } else {
        // No saved position, center on current display
        const [currentX, currentY] = pastilleWindow.getPosition();
        const currentDisplay = screen.getDisplayNearestPoint({ x: currentX, y: currentY });
        const { x: displayX, y: displayY, width, height } = currentDisplay.workArea;
        const x = displayX + Math.round((width - PASTILLE_WINDOW_WIDTH) / 2);
        ...
        userDataManager?.savePastillePosition({ x, y, displayId: currentDisplay.id });
        console.log(`📕 📍 No saved position, centered on current display ${currentDisplay.id} at: ${x}, ${y}`);
      }
      
      // Remove always on top
      pastilleWindow.setAlwaysOnTop(false);
      
      console.log('📕 Pastille collapsed back to normal size');
    }
  });
  ...
  ipcMain.handle('check-python-setup', async () => {
    try {
      ...
      try {
        ...
        if (success) {
          ...
          console.log('🚀 Restarting Python daemon after successful auto setup...');
          try {
            ...
            console.log('✅ Python daemon started successfully after auto setup');
          } catch (daemonError) {
            console.error('⚠️ Python daemon failed to start after auto setup:', daemonError);
    ...
    ipcMain.handle('setup-python-custom', async () => {
      try {
        ...
        if (success) {
          // Restart the Python daemon after successful setup to ensure it gets the model configuration
          console.log('🚀 Restarting Python daemon after successful custom setup...');
          try {
            // Check if daemon is already running and restart it, or start it fresh
            const daemon = await PythonDaemon.getInstance();
            if (daemon.isRunning()) {
            ...
            console.log('✅ Python daemon started successfully after custom setup');
          } catch (daemonError) {
            console.error('⚠️ Python daemon failed to start after custom setup:', daemonError);
            // Don't fail the setup because of daemon startup issues
          }
        }
  ...
  ipcMain.on('toggle-maximize-main-window', () => {
    if (mainWindow) {
  ...
  ipcMain.handle('play-first-run-recording', async (event, filePath) => {
    ...
    try {
      ...
      return { success: false, error: (error as Error).message };
          ...
          // Python environment management - removed as it's not needed for pastille positioning

    // Helper function to calculate pastille position based on config
    function calculatePastillePosition(positionConfig: any): { x: number; y: number; displayId: number } {
      let targetDisplay: Electron.Display;
      
      // Determine target display
      if (positionConfig.displayMode === 'primary') {
        targetDisplay = screen.getPrimaryDisplay();
      } else if (positionConfig.displayMode.startsWith('display-')) {
        const displayId = parseInt(positionConfig.displayMode.replace('display-', ''));
        targetDisplay = screen.getAllDisplays().find(d => d.id === displayId) || screen.getPrimaryDisplay();
      } else {
        // Auto-detect: use current cursor position
        const cursorPos = screen.getCursorScreenPoint();
        targetDisplay = screen.getDisplayNearestPoint(cursorPos);
      }
      
      const { x: displayX, y: displayY, width, height } = targetDisplay.workArea;
      
      let x: number, y: number;
      
      if (positionConfig.mode === 'custom') {
        // Use custom coordinates relative to display
        x = displayX + (positionConfig.x || 0);
        y = displayY + (positionConfig.y || 0);
        
        // Ensure pastille stays within display bounds
        x = Math.max(displayX, Math.min(x, displayX + width - PASTILLE_WINDOW_WIDTH));
        y = Math.max(displayY, Math.min(y, displayY + height - PASTILLE_WINDOW_HEIGHT));
      } else {
        // Center mode
        x = displayX + Math.round((width - PASTILLE_WINDOW_WIDTH) / 2);
        y = displayY + Math.round((height - PASTILLE_WINDOW_HEIGHT) / 2);
      }
      
      return { x, y, displayId: targetDisplay.id };
    }

    // Pastille position IPC handlers
  ipcMain.on('load-pastille-position', (event) => {
    try {
      const position = userDataManager?.loadPastillePosition();
      event.reply('pastille-position-loaded', position);
    } catch (error) {
      console.error('❌ Failed to load pastille position:', error);
      event.reply('pastille-position-loaded', null);
    }
  });

  ipcMain.on('load-pastille-position', (event) => {
    try {
      // Load both position and configuration data
      const position = userDataManager?.loadPastillePosition();
      const config = userDataManager?.loadPastilleConfig();
      
      // Merge position and config data for the settings interface
      const combinedData = {
        x: position?.x,
        y: position?.y,
        displayId: position?.displayId,
        displayMode: config?.displayMode || 'auto',
        mode: config?.mode || 'center'
      };
      
      console.log('📍 Loading pastille position and config:', combinedData);
      event.reply('pastille-position-loaded', combinedData);
    } catch (error) {
      console.error('❌ Failed to load pastille position:', error);
      event.reply('pastille-position-loaded', null);
    }
  ...
  ipcMain.on('load-display-info', (event) => {
    try {
      ...
      event.reply('display-info-loaded', displays);
    } catch (error) {
      console.error('❌ Failed to load display info:', error);
      event.reply('display-info-loaded', []);
    }
  });

  ipcMain.on('preview-pastille-position', (event, positionConfig) => {
    try {
      console.log('👁️ Previewing pastille position:', positionConfig);
      
      if (pastilleWindow) {
        const targetPosition = calculatePastillePosition(positionConfig);
        console.log('📍 Calculated preview position:', targetPosition);
        
        // Temporarily move pastille to preview position
        const originalPosition = pastilleWindow.getPosition();
        pastilleWindow.setPosition(targetPosition.x, targetPosition.y);
        
        // Show pastille if hidden
        if (!pastilleWindow.isVisible()) {
          pastilleWindow.show();
        }
        
        // Briefly highlight the pastille
        pastilleWindow.webContents.send('highlight-pastille');
        
        // Return to original position after preview
        setTimeout(() => {
          pastilleWindow.setPosition(originalPosition[0], originalPosition[1]);
        }, 1500);
      }
    } catch (error) {
      console.error('❌ Failed to preview pastille position:', error);
    }
  });

  ipcMain.on('reset-pastille-position', (event) => {
    try {
      console.log('🔄 Resetting pastille position to center');
      ...
      if (pastilleWindow) {
        ...
        // Reset saved position (only save properties that the method accepts)
        userDataManager?.savePastillePosition({ 
          x, 
          y, 
          displayId: primaryDisplay.id
        });
        
        console.log(`📍 Position reset to center: ${x}, ${y}`);
      }
    } catch (error) {
      console.error('❌ Failed to reset pastille position:', error);
    }
  });
  ...
  ipcMain.on('reset-pastille-position', (event) => {
    try {
      ...
      if (pastilleWindow) {
        ...
        userDataManager?.savePastilleConfig({
          displayMode: 'auto',
          mode: 'center'
        });
        
        console.log(`📍 Position and config reset to defaults: ${x}, ${y}`);
      }
    } catch (error) {
      console.error('❌ Failed to reset pastille position:', error);
    }
  ...
  ipcMain.on('apply-pastille-position', (event, positionConfig) => {
    try {
      console.log('✅ Applying pastille position:', positionConfig);
      ...
      if (pastilleWindow) {
        ...
        // Save the applied position (only save properties that the method accepts)
        const savedPosition = {
          x: targetPosition.x,
          y: targetPosition.y,
          displayId: targetPosition.displayId
        };
        
        userDataManager?.savePastillePosition(savedPosition);
        console.log(`📍 Position applied and saved:`, savedPosition);
      }
    } catch (error) {
      console.error('❌ Failed to apply pastille position:', error);
    }
  });
  ...
  ipcMain.on('apply-pastille-position', (event, positionConfig) => {
    try {
      ...
      if (pastilleWindow) {
        ...
        const savedConfig = {
          displayMode: positionConfig.displayMode,
          mode: positionConfig.mode,
          x: positionConfig.x,
          y: positionConfig.y
        };
        userDataManager?.savePastilleConfig(savedConfig);
        
        console.log(`📍 Position and config applied:`, { savedPosition, savedConfig });
      }
    } catch (error) {
      console.error('❌ Failed to apply pastille position:', error);
    }
